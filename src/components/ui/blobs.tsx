...
export function AnimatedBlobs({ text = "Blobs", className }: { text?: string; className?: string }) {
  const blobStyle = {
  ...
  return (
    <div className={`w-full h-full flex items-center justify-center overflow-hidden bg-black relative ${className || ""}`}>
      <span className="absolute pointer-events-none z-10 text-center text-3xl leading-none font-semibold tracking-tighter whitespace-pre-wrap text-white">
        {text}
      </span>
      <div className="absolute inset-0 flex items-center justify-center" style={{ transform: "scale(0.45)" }}>
